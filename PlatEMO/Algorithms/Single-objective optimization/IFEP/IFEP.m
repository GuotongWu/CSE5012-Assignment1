    ...
    methods
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(Population)
                Offspring  = OperatorIFEP(Problem,Population);
...
function Offspring = OperatorIFEP(Problem,Population)
    %% Parameter setting
    ...
    tau  = 1/sqrt(2*sqrt(D));
    tau1 = 1/sqrt(2*D);
    OffDec1       = PopulationDec + PopulationEta.*trnd(1,N,D);
    OffDec2       = PopulationDec + PopulationEta.*randn(N,D);
    OffEta1       = PopulationEta.*exp(tau1*repmat(randn(N,1),1,D)+tau*randn(N,D));
    OffEta2       = PopulationEta.*exp(tau1*repmat(randn(N,1),1,D)+tau*randn(N,D));
    OffDec        = [OffDec1;OffDec2];
    OffEta        = [OffEta1;OffEta2];
